...
# Tier A census/real/verify, Tier B census/real/verify.
# usage: ./phase.sh <tag> [repros|alu|tierA|tierAreal|tierAverify|tierB|tierBreal|tierBverify ...]
set -u
TAG=$1; shift
...
for w in $what; do
  case $w in
    repros)
      (cd $S/walker/repros && ./run.sh after_$TAG /usr/local/src/nvc/build r30_A r30_G r24_fcvt r23_f2i r22_ffirst r21_dpram r20_fflags r19_bigcat_vars r18_cfn r17_warr r16_bigcat r15_resize_s r10_dynmulti r11_oob r12_casez r13_ternary r14_castsigned r1_fcap r2_nest r3_nba r4_slice_arm r5_pvar_read r6_bitread r7_sra r8_dynslice r9_formal_elem 2>&1 | tee $S/walker/repros/after_$TAG.txt)
      ;;
    alu)
    ...
    tierAverify)
      ...
      ;;
    tierB)
      echo "== Tier B census"; $R $S/cmp/final/exec_tierB tb_exec_replay ${TAG}_tierB_census NVC_ACCEL_RTLIL_CENSUS=1 | grep -E "decline\(s\)|PASS|FAIL|wall|rc=" | grep -v " 0 decline"
      python3 $S/cmp/rtlil_catalog/agg.py $S/cmp/rtlil_catalog/accel_${TAG}_tierB_census.log | tail -25
      ;;
    tierBreal)
      echo "== Tier B real";   $R $S/cmp/final/exec_tierB tb_exec_replay ${TAG}_tierB_real | grep -E "synth|declin|Generated|ACTIVE|PASS|FAIL|wall|rc=|fitted|walker admits" | cut -c1-200
      ;;
    tierBverify)
      echo "== Tier B verify"; $R $S/cmp/final/exec_tierB tb_exec_replay ${TAG}_tierB_verify NVC_ACCEL_VERIFY=1 | grep -E "synth|declin|VERIFY|diverg|PASS|FAIL|wall|rc=" | cut -c1-200
      ;;
  esac
done
