...
def main():
    ...
    print("   baseline: A %.1f ps, B %.1f ps; imbalance %.1f ps" % (d0["a"][0], d0["b"][0], abs(d0["a"][0] - d0["b"][0])))
    ...
    spread0 = abs(d0["a"][0] - d0["b"][0]); mean0 = (d0["a"][0] + d0["b"][0]) / 2
    ...
    def cost(f_, e_, x):
        ...
        return sp / spread0 + 0.05 * mean / mean0 + 0.02 * extra, {"A_ps": d["a"][0], "B_ps": d["b"][0], "spread_ps": sp, "fingers": sum(x.values())}
    ...
    print("== 3. result: %s -> A %.1f ps, B %.1f ps, imbalance %.1f ps (was %.1f); fingers %d -> %d; legal=%s topology_ok=%s violations=%d; %d states" % (
