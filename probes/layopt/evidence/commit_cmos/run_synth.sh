#!/bin/bash
# commit island (rtlmeter-tree Vortex 'mini', flat-netlist island extraction) ->
# IHP SG13G2 stdcells. SAME recipe as ../alu_cmos/run_synth.sh (the E_tree anchor).
# commit_isl.json was produced by extract_island.py (copy in this dir) from
...
D=/usr/local/src/mylex/probes/layopt/evidence/commit_cmos
cd $D/work
yosys -p "read_json $D/commit_isl.json; hierarchy -top commit_isl; synth -top commit_isl -flatten; \
  dfflibmap -liberty $LIB; abc -liberty $LIB; opt_clean; \
  tee -o cmos_stat.txt stat -liberty $LIB; write_verilog -noattr commit.cmos.v" 2>&1 | tee synth.log | tail -30
