#!/bin/bash
# icache island (rtlmeter-tree Vortex 'mini', flat-netlist island extraction) ->
# IHP SG13G2 stdcells. SAME recipe as ../alu_cmos/run_synth.sh (the E_tree anchor).
# icache_isl.json was produced by extract_island.py (copy in this dir) from
...
D=/usr/local/src/mylex/probes/layopt/evidence/icache_cmos
cd $D/work
yosys -p "read_json $D/icache_isl.json; hierarchy -top icache_isl; synth -top icache_isl -flatten; \
  dfflibmap -liberty $LIB; abc -liberty $LIB; opt_clean; \
  tee -o cmos_stat.txt stat -liberty $LIB; write_verilog -noattr icache.cmos.v" 2>&1 | tee synth.log | tail -30
