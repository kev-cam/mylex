...
in, and each cut wire's pieces are reconnected by the router with the far
piece as the only landing; any failure rolls everything back. Exercised by
`test_move_pr_wire` (a sealed bare nand2 row).
...
gcd: 13 of 14 boundaries dissolved (2.92 µm freed, 0.46 µm given back as a whole site, the rest gaps beside the merged groups; a minute on three-row windows), 13 macros, 5 other instances moved; routed to completion, 0 DRC, 5563 µm of wire (base 5566); device-level topology equal to the original routed gcd; KLayout isomorphic. Evidence: `evidence/gcd_merged.lef`, `gcd_merged.def`,
